...
class NTA_Whatsapp_Setting {
    ...
    public function __construct() {
        ...
        add_action('admin_menu', function() {
            ...
            add_submenu_page('nta_whatsapp', 'Add New account', 'Add New account', 'edit_posts', $edit_account_link);
            add_submenu_page('nta_whatsapp', 'Floating Widget', 'Floating Widget', 'edit_posts', 'floating-widget-whatsapp', [$this, 'tab_menu_page']);
            add_submenu_page('nta_whatsapp', 'Settings', 'Settings', 'manage_options', 'nta_whatsapp', [$this, 'create_page_setting_widget']);
        });
    ...
    public function page_display_settings_section_callback() {
        ...
        if (!$option) {
            ...
            $option['nta_widget_status'] = 'ON';
        } else {
            $option['widget_name'] = getValueOrDefault($option['widget_name'],'Start a Conversation');
            $option['widget_position'] = getValueOrDefault($option['widget_position'],'right');
            $option['widget_label'] = getValueOrDefault($option['widget_label'], 'Need Help? <strong>Chat with us</strong>');
            $option['widget_responseText'] = getValueOrDefault($option['widget_responseText'], 'The team typically replies in a few minutes.');
            $option['text_color'] = getValueOrDefault($option['text_color'], '#fff');
            $option['back_color'] = getValueOrDefault($option['back_color'], '#2db742');
            $option['widget_description'] = getValueOrDefault($option['widget_description'], 'Hi! Click one of our member below to chat on <strong>Whatsapp</strong>');
            $option['nta_widget_status'] = getValueOrDefault($option['nta_widget_status'] ,'ON');
        }
    ...
    public function save_woobutton_setting($input) {
        ...
        $new_input['nta_woo_button_position'] = $_POST['nta_woo_button_position'];
    ...
    public function save_setting($input) {
        $new_input = [];
        $new_input['widget_name'] = $_POST['widget_name'];
        $new_input['widget_label'] = $_POST['widget_label'];
        $new_input['text_color'] = $_POST['text_color'];
        $new_input['back_color'] = $_POST['back_color'];
        $new_input['widget_position'] = $_POST['widget_position'];
        $new_input['widget_description'] = wp_kses_post($_POST['widget_description']);
        $new_input['widget_responseText'] = $_POST['widget_responseText'];
            
        if (isset($_POST['nta_widget_status'])) {
            $new_input['nta_widget_status'] = 'ON';
        }
        return $new_input;
    }
    ...
    public function add_account() {
        if (isset($_POST['add'])) {
            ...
            $clone_meta_value['account_name'] = $_POST['account_name'];
