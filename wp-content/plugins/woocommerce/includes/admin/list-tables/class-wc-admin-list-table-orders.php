...
class WC_Admin_List_Table_Orders extends WC_Admin_List_Table {
	...
	public function __construct() {
		...
		#MOD envios
		add_action( 'admin_notices', array( $this, 'add_progress_bar_servientrega' ) );
		#MOD confirmadas
		add_action( 'admin_notices', array( $this, 'add_progress_bar_confirmed' ) );
	}

	public function add_progress_bar_confirmed(){

		if (isset($_REQUEST['order_confirmed'])) {
			echo '<div class="notice notice-warning is-dismissible">
					Progreso de completar ordenes<br><br>
					<div id="progressbar"></div>
					<br>
					<button class="button runproccess" onclick="runProcess(0)">Correr proceso</button>
					<br><br>
					<script>
				...
				progressbar.progressbar({
				    	...
				    	complete: function(){
				    		setTimeout(function() {
				    				...
				    				window.location = removeParam("order_confirmed",window.location.href);
				    			});
				    	}
					});

					var orders = "'.$_REQUEST['order_confirmed'].'";
					var order_array = orders.split("|");
					var total_orders =  order_array.length;
					function runProcess(i) {
						jQuery(".runproccess").hide();
						console.log(i)
						if(i == total_orders){
							return true;
						}
						...
						jQuery.ajax({
							...
							url: ajaxurl,
							async: true	,
							data: { action: "ajax_confirm_orders", order: order_array[i] }
						})
						.done(function( msg ) {
							i++;
							var percent = ((i * 100) / total_orders);
							console.log("percent;"+percent);
							progressbar.progressbar( "value", percent);
							setTimeout(runProcess(i), 2500);
							return true;
						});
					}
	...
	public function add_progress_bar_servientrega(){

		if (isset($_REQUEST['order_guide'])) {
			echo '<div class="notice notice-warning is-dismissible">
	            Progreso de Guias<br><br>
				<div id="progressbar"></div>
				<br>
				<button class="button runproccess" onclick="runProcess(0)">Correr proceso</button>
				<br><br>
				<script>
					...
					progressbar.progressbar({
							...
							complete: function(){
								setTimeout(function() {
										alert("PROCESO COMPLETADO");
										window.location = removeParam("order_guide",window.location.href);
									});
							}
						});

					var orders = "'.$_REQUEST['order_guide'].'";
					var order_array = orders.split("|");
					var total_orders =  order_array.length;
					function runProcess(i) {
						jQuery(".runproccess").hide();
						...
						jQuery.ajax({
							...
							url: ajaxurl,
							async: true	,
							data: { action: "new_servientrega_guide", order: order_array[i] }
						})
						.done(function( msg ) {
							i++;
							var percent = ((i * 100) / total_orders);
						...
						.fail(function() {
							alert( "ORDEN "+ order_array[i] + " FALLO" );
							i++;
							var percent = ((i * 100) / total_orders);
							console.log("percent;"+percent);
							progressbar.progressbar( "value", percent);
