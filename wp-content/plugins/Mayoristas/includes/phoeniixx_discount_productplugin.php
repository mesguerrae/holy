<?php if ( ! defined( 'ABSPATH' ) ) exit;
	...
	function phoen_dpad_custom_tab_options_discounts() {
		...
		$type = get_post_meta($post->ID,'phoen_woocommerce_discount_type', true); 
		...
		<script>
			...
			jQuery(document).ready(function(){
				...
				jQuery('.phoe_add_disc_more').click(function(){
					...
					var type = jQuery("input[name$='discount_type']:checked").val();
					var a = jQuery('#phoeniixx_discount_div_'+type).html();
					jQuery('.discount_type_'+type).append(a);

				});

				jQuery("input[name$='discount_type']").click(function() {
			        var type = jQuery(this).val();
			        jQuery(".discount_type_qty,.discount_type_price").hide();
			        jQuery(".discount_type_" + type).show();
			    });


			    jQuery(".discount_type_qty,.discount_type_price").hide();
			    jQuery(".discount_type_" + "<?php echo $type; ?>").show();
			    jQuery('input[name=discount_type][value="<?php echo $type; ?>"]').attr('checked', true);
...
<body>
	...
	<div id="phoeniixx_discount_div_qty" style="display:none;">
	...
	<div id="phoeniixx_discount_div_price" style="display:none;">
	
		<div class="phoeniixx_discount_min_max_div">

			<input type="number" placeholder="Min Price" name="min_price[]" min=0  class="min_price" value="">

			<input type="number"  placeholder="Max Price" name="max_price[]" min=0 class="max_price" value="">

			<input type="number"  placeholder="Required Qty" name="qty[]" min=0 class="qty" value="">

			<input type="number" step='any' placeholder="Discount Value" name="discount_price[]" min=0 class="discount" value="">

			<input type="text" step='any' placeholder="Label" name="label[]" min=0 class="" value="">
		...
		<div id="accordion1" class="phoen_main_accordian">
					...
					<div class="phoen_user_roles" >
						...
						<select class="user_role" name="user_role[]" multiple="multiple" >
							...
							<option value="wholesaler" <?php if(isset($val[0]['user_role']) && in_array('wholesaler', $val[0]['user_role'])) echo 'selected';?>><?php _e('Mayorista','phoen-dpad'); ?></option>
							
						</select>
						<br>
						<input type="radio" id="qty" name="discount_type" value="qty">
						<label for="qty">Qty</label><br>
						<input type="radio" id="price" name="discount_type" value="price">
					...
					<div class="phoen_expiry" <?php if(isset($val[0]['never_expire']) && $val[0]['never_expire']=='1') echo 'style=display:none;';?>>
						...
						<div class="phoeniixx_discount_html_content_div discount_type_qty">
							...
							<?php 
							if(isset($val[0]) && is_array($val) && $type == 'qty'){
						...
						<div class="phoeniixx_discount_prince_html_content_div discount_type_price">
						
							<div class="phoen_mindiv_label">
								<label><b><?php _e('Min price','phoen-dpad'); ?></b></label> 
								<label><b><?php _e('Max price','phoen-dpad'); ?></b></label>
								<label><b><?php _e('Qty','phoen-dpad'); ?></b></label> 
								<label><b><?php _e('Product price','phoen-dpad'); ?></b></label>
								<label><b><?php _e('Label','phoen-dpad'); ?></b></label>
							</div>
							<?php 
							if(isset($val[0]) && is_array($val) && $type == 'price'){
							//if(isset($val) && count($val)>0){
								for($i=0;$i<count($val);$i++)
								...
								{						
									...
									<div class="phoeniixx_discount_min_max_div" > 
										...
										<input type="number" placeholder="Min Price" name="min_price[]" min=0 class="min_price" value="<?php echo isset($val[$i]['min_price'])?$val[$i]['min_price']:''; ?>" />

										<input type="number" placeholder="Max Price" name="max_price[]" min=0  class="max_price" value="<?php echo isset($val[$i]['max_price'])?$val[$i]['max_price']:''; ?>" />

										<input type="number" step='any' placeholder="Qty" name="qty[]" min=0 class="discount" value="<?php echo isset($val[$i]['qty'])?$val[$i]['qty']:''; ?>" />

										<input type="number" step='any' placeholder="Discount Value" name="discount_price[]" min=0 class="discount" value="<?php echo isset($val[$i]['discount_price'])?$val[$i]['discount_price']:''; ?>" />

										<input type="text" step='any' placeholder="Label" name="label[]" min=0 class="discount" value="<?php echo isset($val[$i]['label'])?$val[$i]['label']:''; ?>" />
								...
								?>
									<div class="phoeniixx_discount_min_max_div"> 
										...
										<input type="number" placeholder="Min Quantity" name="min_price[]" min=0 class="min_val" value="" />

										<input type="number"  placeholder="Max Quantity" name="max_price[]" min=0 class="max_val" value="" />
	...
	function phoen_dpad_process_product_meta_custom_tab_discounts( $post_id ) {
		...
		update_post_meta( $post_id, 'phoen_woocommerce_discount_type', $discount_type );
