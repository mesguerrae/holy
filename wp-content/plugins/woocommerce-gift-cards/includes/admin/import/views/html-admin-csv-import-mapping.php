<?php
/**
 * Admin View: CSV Import mappings.
 *
...
<form class="wc-progress-form-content woocommerce-importer" method="post" action="<?php echo esc_url( $this->get_next_step_link() ); ?>">
	<header>
		<h2><?php esc_html_e( 'Map CSV fields to gift card data', 'woocommerce-gift-cards' ); ?></h2>
		<p><?php esc_html_e( 'Select fields from your CSV file to map against gift card fields, or to ignore during import.', 'woocommerce-gift-cards' ); ?></p>
	</header>
		...
		<table class="widefat wc-importer-mapping-table">
			...
			<tbody>
				<?php foreach ( $headers as $index => $name ) : ?>
					<?php $mapped_value = isset( $mapped_items[ $index ] ) ? $mapped_items[ $index ] : ''; ?>
					<tr>
