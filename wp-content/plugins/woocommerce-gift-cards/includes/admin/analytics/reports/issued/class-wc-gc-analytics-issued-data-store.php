<?php
/**
 * REST API Reports giftcards issued datastore
 *
 ...
 * @version 1.8.0
 */
class WC_GC_Analytics_Issued_Data_Store extends WC_GC_Analytics_Data_Store {
	 ...
	 * @var string
	 */
	protected $cache_key = 'giftcards_issued';
	...
	protected function assign_report_columns() {
		...
		$this->report_columns = array(
			...
			'issued_balance'    => 'amount as issued_balance',
			'expire_date'       => '( CASE WHEN expire_date = 0 THEN NULL ELSE FROM_UNIXTIME(expire_date) END ) as expire_date',
		);
	}
	...
	protected function add_sql_query_params( $query_args ) {
		...
		// Join main giftcards table.
		$this->subquery->add_sql_clause( 'join', "JOIN {$wpdb->prefix}woocommerce_gc_cards ON {$wc_gc_activities_table}.gc_id = {$wpdb->prefix}woocommerce_gc_cards.id" );
	}
	...
	protected function normalize_order_by( $order_by ) {
		...
		if ( 'date' === $order_by ) {
			return self::get_db_table_name() . '.date';
		} elseif ( 'expire_date' === $order_by ) {
			return "{$wpdb->prefix}woocommerce_gc_cards.expire_date";
		} elseif ( 'issued_balance' === $order_by ) {
			return self::get_db_table_name() . '.amount';
		}
	...
	protected function initialize_queries() {
		...
		$this->subquery->add_sql_clause( 'where', "AND {$table_name}.`type` = 'issued'" );
	}
}
