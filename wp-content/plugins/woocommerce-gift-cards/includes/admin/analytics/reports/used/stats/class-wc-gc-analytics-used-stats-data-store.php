...
/**
 * WC_GC_Analytics_Used_Stats_Data_Store class.
 *
 * @version 1.8.0
 */
class WC_GC_Analytics_Used_Stats_Data_Store extends WC_GC_Analytics_Used_Data_Store {
	 ...
	 * @var string
	 */
	protected $cache_key = 'giftcards_used_stats';
	 ...
	 * @var string
	 */
	protected $context = 'giftcards_used_stats';
	...
	protected function assign_report_columns() {
		...
		$this->report_columns = array(
			...
			'used_amount'     => "SUM( CASE WHEN `type` = 'used' THEN `{$table_name}`.`amount` END ) as used_amount",
			'refunded_amount' => "SUM( CASE WHEN `type` = 'refunded' THEN `{$table_name}`.`amount` END ) as refunded_amount",
			'net_amount'      => "SUM( CASE WHEN type = 'used' THEN `{$table_name}`.`amount` WHEN `type` = 'refunded' THEN -`{$table_name}`.`amount` END ) as net_amount",
		);
	}
	...
	public function get_data( $query_args ) {
		...
		if ( false === $data ) {
			...
			if ( null === $totals ) {
				return new \WP_Error( 'woocommerce_analytics_giftcards_used_stats_result_failed', __( 'Sorry, fetching revenue data failed.', 'woocommerce-gift-cards' ) );
			}
	...
	protected function initialize_queries() {
		...
		$this->total_query->add_sql_clause( 'where', 'AND type IN ( "used", "refunded" )' );

		$this->interval_query = new SqlQuery( $this->context . '_interval' );
		$this->interval_query->add_sql_clause( 'from', self::get_db_table_name() );
		$this->interval_query->add_sql_clause( 'where', 'AND type IN ( "used", "refunded" )' );
