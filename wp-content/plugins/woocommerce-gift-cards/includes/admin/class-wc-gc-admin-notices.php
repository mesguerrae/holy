...
class WC_GC_Admin_Notices {
	...
	private static $maintenance_notice_types = array(
		...
		'marketing_menu'     => 'marketing_menu_notice'
	);
	...
	/**
	 * Add 'marketing_menu' notice.
	 *
	 * @since 1.6.0
	 */
	public static function marketing_menu_notice() {
		...
		if ( ! WC_GC_Core_Compatibility::is_wc_version_gte( '4.4' ) ) {
			return;
		}
		...
		$notice = sprintf(
			...
			__( 'Heads up! The <strong>WooCommerce > Gift Cards</strong> menu item has been moved to %1$sMarketing > Gift Cards%2$s.', 'woocommerce-gift-cards' ),
			'<a href="' . esc_url( admin_url( 'admin.php?page=gc_giftcards' ) ) . '">',
			'</a>'
		);
		self::add_dismissible_notice( $notice, array( 'type' => 'info', 'dismiss_class' => 'marketing_menu' ) );
	}
