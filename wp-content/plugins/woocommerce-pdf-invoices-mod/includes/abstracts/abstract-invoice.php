...
abstract class BEWPI_Abstract_Invoice extends BEWPI_Abstract_Document {
	...
	public function get_columns() {
		...
		$this->add_column( $columns, 'description', __( 'Description', 'woocommerce' ) );
		$this->add_column( $columns, 'quantity', __( 'Qty', 'woocommerce' ) );
		$this->add_column( $columns, 'unitary value', __( 'Price', 'woocommerce' ) );
	...
	/**
	 * Adds unitary value to columns data array.
	 *
	 * @param array  $data    line item data.
	 * @param int    $item_id item ID.
	 * @param object $item    item object.
	 * @param bool   $ex_tax  Excluding tax.
	 */
	public function add_unitary_value( &$data, $item) {
		$data['unitary_value'] =  wc_price(($item->get_total() / $item->get_quantity()));
	}
	...
	public function get_columns_data( $items = array() ) {
		...
		foreach ( $items as $item_id => $item ) {
			...
			$this->add_unitary_value( $row, $item );
	...
	protected function add_order_item_totals_discount_row( &$total_rows, $tax_display ) {
		if ( $this->order->get_total_discount() > 0 ) {
			$total_rows['discount'] = array(
				'label' => __( 'Descuento:', 'woocommerce-pdf-invoices' ),
	...
	protected function add_order_item_totals_shipping_row( &$total_rows, $tax_display ) {
		if ( $this->order->get_shipping_method() ) {
			$total_rows['shipping'] = array(
				'label' => __( 'Envío:', 'woocommerce-pdf-invoices' ),
