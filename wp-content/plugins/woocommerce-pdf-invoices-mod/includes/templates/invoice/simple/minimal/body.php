...
<div class="title">
	...
	<div class="watermark">
		...
		if ( WPI()->get_option( 'template', 'show_payment_status' ) && $order->is_paid() ) {
			printf( '<h2 class="green">%s</h2>', esc_html__( 'Pagado', 'woocommerce-pdf-invoices' ) );
		}
...
<table cellpadding="0" cellspacing="0">
	<tr class="information">
		...
		<td>
			<?php
			printf( '<strong>%s</strong><br />', esc_html__( 'Datos de cliente:', 'woocommerce-pdf-invoices' ) );
		...
		<td>
			...
			if ( WPI()->get_option( 'template', 'show_ship_to' ) && ! WPI()->has_only_virtual_products( $order ) && ! empty( $formatted_shipping_address ) ) {
				printf( '<strong>%s</strong><br />', esc_html__( 'Datos de envío:', 'woocommerce-pdf-invoices' ) );
