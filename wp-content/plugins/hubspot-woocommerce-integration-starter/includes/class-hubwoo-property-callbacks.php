...
class HubWooPropertyCallbacks{
	...
	public function hubwoo_user_meta( $key ) {
	    ...
	    $this->_cache[ 'marketing_newsletter' ] = $this->hubwoo_format_array( $optin_sources );
		...
		if( is_array( $customer_orders ) && count( $customer_orders ) ) {
			...
			foreach( $customer_orders as $order_id ) {
				...
				if( !$counter ) {
					...
					$this->_cache[ 'last_order_fulfillment_status' ] = "wc-".$order->get_status();
						
					$this->_cache[ 'last_order_shipment_date' ] = apply_filters( 'hubwoo_order_shipment_date', '', $order_id );

					$this->_cache[ 'last_order_tracking_number' ] = $order->get_order_number();
					...
					$this->_cache[ 'last_product_bought' ] = $this->hubwoo_format_array( $last_product_bought );

					$this->_cache[ 'last_products_bought' ] = $this->hubwoo_format_array( $last_products_bought );
					
					$this->_cache[ 'last_skus_bought' ] = $this->hubwoo_format_array( $skus_bought );
					
					$this->_cache[ 'last_categories_bought' ] = $this->hubwoo_format_array( $categories_bought );
					
					$this->_cache[ 'last_product_types_bought' ] = $this->hubwoo_format_array( $product_types_bought );
			...
			$this->_cache[ 'skus_bought' ] = $this->hubwoo_format_array( $skus_bought );
				
			$this->_cache[ 'categories_bought' ] = $this->hubwoo_format_array( $categories_bought );
			
			$this->_cache[ 'product_types_bought' ] = $this->hubwoo_format_array( $product_types_bought );
			$this->_cache['products_bought' ] = $this->hubwoo_format_array( $products_bought );
	 ...
	 * 
	 * @param  @key  subscription properties
	 * @since  1.0.0
	 */
			...
			foreach( $customer_orders as $counter => $order_id ) {
				...
				if( is_array( $order_items ) && count( $order_items ) ) {
					...
					if( count( $subs_products ) ) {
						...
						$this->_cache[ 'last_subscription_products' ] = $this->hubwoo_format_array( $subs_products );
					}
				...
				if( !empty( $order_data["schedule_trial_end"] ) ) {
					...
					$this->_cache[ 'last_subscription_trial_end_date' ] = $this->hubwoo_set_utc_midnight( $order_data["schedule_trial_end"]->getTimestamp() );	
				}

				if( !empty( $order_data["schedule_end"] ) && $order_data["schedule_end"] instanceof WC_DateTime ) {

					$this->_cache[ 'last_subscription_next_payment_date' ] = $this->hubwoo_set_utc_midnight( $order_data["schedule_end"]->getTimestamp() );
				}
				...
				if( !empty( $date_created ) ) {
					...
					$this->_cache[ 'last_subscription_order_creation_date' ] = $this->hubwoo_set_utc_midnight( strtotime( $date_created ) );
				}
				...
				if( !empty( $date_paid ) ) {
					...
					$this->_cache[ 'last_subscription_order_paid_date' ] = $this->hubwoo_set_utc_midnight( strtotime( $date_paid ) );
				}
				...
				if( !empty( $date_completed ) ) {
					...
					$this->_cache[ 'last_subscription_order_completed_date' ] = $this->hubwoo_set_utc_midnight( strtotime( $date_completed ) );
				}
				...
				if( !empty( $last_order_creation_date ) ) {
					...
					$this->_cache[ 'related_last_order_creation_date' ] = $this->hubwoo_set_utc_midnight( strtotime( $last_order_creation_date ) );
				}
				...
				if( !empty( $last_order_date_paid ) ) {
					...
					$this->_cache[ 'related_last_order_paid_date' ] = $this->hubwoo_set_utc_midnight( strtotime( $last_order_paid_date ) );
				}
				...
				if( !empty( $last_order_completion_date ) ) {
					...
					$this->_cache[ 'related_last_order_completed_date' ] = $this->hubwoo_set_utc_midnight( strtotime( $last_order_completion_date ) );
				}
