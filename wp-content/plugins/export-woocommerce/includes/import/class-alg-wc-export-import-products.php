...
if ( ! class_exists( 'Alg_WC_Export_Import' ) ) :
	class Alg_WC_Export_Import {
		...
		function __construct() {
			...
			add_action( 'woocommerce_settings_tabs_alg_wc_export_import', array($this, 'settings_tab') );
			add_action( 'woocommerce_update_options_alg_wc_export_import', array($this, 'update_settings') );
		...
		function add_settings_tab( $settings_tabs ) {
			$settings_tabs['alg_wc_export_import'] = __( 'Import', 'export-woocommerce' );
		...
		function update_settings() {
			if(isset($_REQUEST['alg_export_upload_actioned']) && !empty($_REQUEST['alg_export_upload_actioned']) && $_REQUEST['alg_export_upload_actioned']=='run_import')
			{
				...
				wp_safe_redirect( admin_url( 'admin.php?page=wc-settings&tab=alg_wc_export_import&importdone=1' ) );
			}
			else
			{
				$this->save_file_to_tmp();
				wp_safe_redirect( admin_url( 'admin.php?page=wc-settings&tab=alg_wc_export_import&uploadedtmp=1' ) );
			}
		}
